...
const ck_secret = config.appSecret

const dictionary = 'ab!@#$ijklmcdefghBCWXYZ01234DEFGHnopqrstuvwxyzAIJKLMNOPQRSTUV56789%^&*()';
//	混淆表
const encodeTableKey = [
    0x41, 0x92, 0x53, 0x2a, 0xfc, 0xab, 0xce, 0x26, 0x0d, 0x1e, 0x99, 0x78, 0x00, 0x22, 0x99, 0xde, 0x41, 0x92, 0x53, 0x2a, 0x23, 0xa1, 0xf5, 0x46, 0x72, 0x9c, 0xdb, 0x31, 0x13,
    0x67, 0x59, 0x3a, 0x1f, 0x49, 0xc3, 0x21, 0x8d, 0x5e, 0x15, 0xb2
];
...
function createBase64(str: string): string {
    let sha256 = CryptoJS.HmacSHA256(str, ck_secret);
    let base64 = CryptoJS.enc.Base64.stringify(sha256);
    return base64;
}

//	加密
function _encrypted(str: string) {
    let index_array = [];
    for (let i = 0, len = str.length; i < len; i++) {
        let index = dictionary.indexOf(str[i]);
        let target = parseInt(index.toString()) << 2;
        index_array.push(target);
    }
    return index_array.toString();
}

//	解密
function _decrypt(sec: string) {
    let array = sec.split(',');
    let result = '';
    array.forEach(function (item) {
        let index = parseInt(item) >> 2;
        let target = dictionary.charAt(index);
        result += target;
    });
    return result.trim();
}
...
//	crc32加密
function _encryptCrc32(key: string, random: number | string) {
    const hexStr = random.toString(16).padStart(2, '0');
    const byteArray = hexStringToByteArray(hexStr + key);
    let crc = 0 ^ -1;
    for (let i = 0; i < byteArray.byteLength; i++) {
        crc = (crc >>> 8) ^ table[(crc ^ byteArray[i]) & 0xff];
    }
    return (((crc ^ -1) >>> 0) >>> 0).toString(16).padStart(8, '0');
}
...
//	按位 异或
function _encryptXor(source: Uint8Array, key: number) {
    const len = source.length;
    const encryptResult = new Uint8Array(len);
    for (let i = 0; i < len; i++) {
        encryptResult[i] = source[i] ^ key;
    }
    return encryptResult;
}

//	使用EnCodeTable 按位异或
function _encryptXorByTable(source: number[], sequence: number) {
    const len = source.length;
    let seq = sequence;
    const encryptResult: number[] = [];
    for (let i = 0; i < len; i++) {
        // console.log(`key ====> ${encodeTableKey[seq % encodeTableKey.length]} ===> ${sequence} ===> ${seq}`);
        encryptResult[i] = source[i] ^ encodeTableKey[seq % encodeTableKey.length];
        seq++;
    }
    // console.log("🚀 ~ file: cryptoUtils.ts:72 ~ _encryptXorByTable ~ encryptResult:", encryptResult)
    return encryptResult;
}
//	arrayBuffer to 16hex
function _ab2hex(buffer: ArrayBuffer) {
    var hexArr = Array.prototype.map.call(new Uint8Array(buffer), function (bit: number) {
...
//	生成 8位 16进制 字符串
function _createRandomAddr() {
    let addr = '';
    for (let i = 0; i < 4; i++) {
        addr += _getRandomHex().toString(16).padStart(2, '0');
    }
    return addr;
}
...
export default {
    ...
    decrypt: _decrypt,
    encryptCrc32: _encryptCrc32,
    encryptXor: _encryptXor,
    encryptXorByTable: _encryptXorByTable,
    ab2hex: _ab2hex,
    getRandomHex: _getRandomHex,
    createRandomAddr: _createRandomAddr
};
